...
void runInvertedPendulum(){
	...
	float const h_angle=0.002;
    float const h_x=0.001;
...
//                 Sleep(100);  // 延迟100毫秒
				 ...
				 newState.angle_dot = prevState.angle_dot + (h_angle * newState.angle_double_dot);
				 newState.angle = prevState.angle + (h_angle * newState.angle_dot);
				 newState.F = prevState.F;
				 newState.x_double_dot = calc_horizontal_acceleration(prevState);
				 newState.x_dot = prevState.x_dot + (h_x * newState.x_double_dot);
		         newState.x = prevState.x + (h_x * newState.x_dot);
